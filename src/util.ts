...
export function assertValidTarget(input: string, what = 'host'): string {
...
export function runProcess(binary: string, args: string[], timeoutMs: number): Promise<ProcessResult> {
...
export function parseRttSummary(line: string): { min: number; avg: number; max: number } | undefined {
...
export function parsePacketSummary(text: string): {
  transmitted: number
  received: number
  lossPercent: number
} | undefined {
