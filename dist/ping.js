 ...
 */
import { assertValidTarget, parsePacketSummary, parseRttSummary, round2, runProcess } from "./util.js";
/** Build the platform-specific argument array for the ping binary. */
export function pingArgs(host, count, timeoutSec) {
...
export function parseResolvedAddress(text) {
...
function pingTimeoutMs(count, timeoutSec) {
    return Math.max(timeoutSec * 1000 * (count + 2), 10_000);
}
/** Ping a host `count` times and summarize the result. */
export async function pingHost(hostInput, count, timeoutSec) {
