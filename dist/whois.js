 ...
 */
export function fallbackWhoisServer(host) {
 ...
 */
export function rawWhoisQuery(server, query, timeoutMs, port = 43) {
...
export function parseIanaReferral(text) {
...
export function parseWhoisSummary(text) {
    const summary = { statuses: [], nameServers: [] };
    const firstOf = (patterns) => {
    ...
    if (expiryDate !== undefined)
        summary.expiryDate = expiryDate;
    for (const pattern of [/^\s*status:\s*(.+)$/img, /^\s*domain\s+status:\s*(.+)$/img]) {
        for (const match of text.matchAll(pattern)) {
            const value = (match[1] ?? '').trim();
            if (value !== '' && !summary.statuses.includes(value))
                summary.statuses.push(value);
        }
    }
    const seen = new Set();
 ...
 */
export async function whoisLookup(domainInput, server, timeoutMs) {
